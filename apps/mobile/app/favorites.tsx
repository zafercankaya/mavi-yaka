...
import { fetchFavorites, toggleFavorite, FavoriteFilter } from '../src/api/favorites';
...
const SEGMENT_KEYS: FavoriteFilter[] = ['active', 'upcoming', 'past'];

export default function FavoritesScreen() {
  const [filter, setFilter] = useState<FavoriteFilter>('active');
  ...
  const { data: favorites, isLoading, refetch } = useQuery({
    queryKey: ['favorites', filter, market],
    queryFn: () => fetchFavorites(filter),
    enabled: isAuthenticated,
  });

  const removeMutation = useMutation({
    mutationFn: (campaignId: string) => toggleFavorite(campaignId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['favorites'] });
    },
  });

  const handleRemove = useCallback((campaignId: string) => {
    removeMutation.mutate(campaignId);
  }, [removeMutation]);
              ...
              >
                <View style={styles.cardContent}>
                  <View style={styles.cardLeft}>
                    {c.brand && (
                      <Text style={[styles.brand, frozen && { color: Colors.textLight }]}>{c.brand.name}</Text>
                    )}
                    ...
                    <View style={styles.meta}>
                      ...
                      {c.discountRate != null && !frozen && (
                        <View style={styles.discountBadge}>
                          <Text style={styles.discountText}>%{c.discountRate}</Text>
                        </View>
                      )}
                      {c.endDate && !frozen && (
